...
class BASIC_ERROR : public std::exception
      { private :
        ...
        : std::exception(){ function=ThrownIn+WHICH;
                            object=InObject+WHO;
                            char buffer[7]; snprintf(buffer,7,"%d",WHERE);
        ...
        void ChangeWhere(const int &WHERE){ char buffer[7]; snprintf(buffer,7,"%d",WHERE); line=AtLine+std::string(buffer);}
        void ChangeLine(const int &WHERE){ char buffer[7]; snprintf(buffer,7,"%d",WHERE); line=AtLine+std::string(buffer);}
